...
function App() {
  ...
  function handleFiles(event) {
    ...
    const added = selected.map(newFile)
  ...
  function updateFile(id, key, value) {
    setFiles((current) => current.map((file) => file.id === id ? { ...file, [key]: key === 'pages' || key === 'copies' ? Math.max(1, Number(value) || 1) : value } : file))
  }
  ...
  return (
    <div className="app-shell">
      {view === 'customer' ? <main className="chat-only-page"><section className="chat-card"><div className="chat-head"><div className="bot-avatar">D</div><div><strong>Doxie</strong><span><i /> Online · your print assistant</span></div><div className="chat-menu-wrap"><button type="button" className="chat-menu-button" aria-label="Open menu" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}>•••</button>{menuOpen && <div className="chat-menu"><button type="button" onClick={() => { setView('owner'); setMenuOpen(false) }}>Owner login <span>↗</span></button></div>}</div></div><div className="chat-body"><div className="chat-welcome"><span className="spark">✳</span><strong>Print it. Pick it up.</strong></div><div className="date-divider"><span>Today</span></div>{messages.map((message, index) => <div className={`message-row ${message.from}`} key={`${message.text}-${index}`}><div className="bubble">{message.text}<small>{message.time} {message.from === 'user' && <b>✓✓</b>}</small></div></div>)}

        {stage === 'upload' && <div className="chat-action-card"><p>Start with your document</p><label className="big-upload"><span>＋</span><strong>Upload files</strong><small>PDF, DOC, DOCX · select one or many</small><input type="file" multiple accept=".pdf,.doc,.docx" onChange={handleFiles} /></label></div>}
        ...
        {stage === 'payment' && <div className="payment-card"><div><span className="section-kicker">ORDER TOTAL</span><strong>{money(total)}</strong><small>{fileCount} file{fileCount > 1 ? 's' : ''} · includes every page and copy</small><label className="mobile-label">Mobile number<input type="tel" inputMode="numeric" maxLength="10" value={mobile} onChange={(event) => setMobile(event.target.value.replace(/\D/g, ''))} placeholder="10-digit number" aria-label="Mobile number for pickup token" /></label></div><button className="pay-button" type="button" onClick={pay}>Pay securely <span>→</span></button><p>Mock payment now · token SMS ready · Razorpay integration ready</p></div>}
      ...
      </div><div className="chat-foot"><label className="upload-button" title="Upload more documents"><span>＋</span><input type="file" multiple accept=".pdf,.doc,.docx" onChange={handleFiles} /> Add files</label><form onSubmit={sendMessage}><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Ask Doxie anything..." aria-label="Message Doxie" /><button type="submit" aria-label="Send message">↑</button></form></div></section></main> : <main className="owner-layout"><div className="owner-title"><div><span className="section-kicker">STORE OPERATIONS</span><h1>Good morning, Arjun.</h1><p>Here’s what’s happening at Doxie today.</p></div><button className="outline-button" type="button">＋ New order</button></div><div className="stat-grid"><div><span>Orders today</span><strong>24</strong><small className="up">↗ 18% vs yesterday</small></div><div><span>In progress</span><strong>07</strong><small>2 need attention</small></div><div><span>Revenue today</span><strong>₹2,840</strong><small className="up">↗ 12% vs yesterday</small></div><div><span>Avg. turnaround</span><strong>18<span> min</span></strong><small>Last 7 days</small></div></div><div className="owner-grid"><section className="orders-table"><div className="table-head"><div><span className="section-kicker">LIVE QUEUE</span><h2>Recent orders</h2></div><button className="filter-button" type="button">Today⌄</button></div>{orders.map((order) => <div className="order-row" key={order.token}><div className="order-token">{order.token}</div><div className="order-name"><strong>{order.name}</strong><span>{order.detail}</span></div><span className={`status-badge ${order.tone}`}><i /> {order.status}</span><button className="dots-button" type="button">•••</button></div>)}</section><section className="verify-panel"><span className="section-kicker">COUNTER TOOL</span><h2>Verify a token</h2><p>Confirm a student's order before handing it over.</p><div className="token-input"><input placeholder="e.g. DX-2048" aria-label="Token to verify" /><button type="button">Check</button></div><div className="verification-card"><span className="verified-icon">✓</span><div><strong>DX-2048 is ready</strong><span>Digital Marketing Notes.pdf · 12 pages</span></div></div></section></div></main>}
    </div>
  )
}
